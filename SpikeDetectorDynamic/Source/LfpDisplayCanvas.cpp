...
void LfpDisplayCanvas::updateScreenBuffer()
{
	...
	if (true)
	{
		...
		int maxSamples = lfpDisplay->getWidth() - leftmargin; //maximum pixel that can be drawn
			...
			if (valuesNeeded > 0 && valuesNeeded < 1000000)
			{
				for (int i = 0; i < valuesNeeded; i++) // also fill one extra sample for line drawing interpolation to match across draws
				{
					...
					if (!lfpDisplay->isPaused)
					{
                        ...
                        // - copy data from displaybuffer to screenbuffer
						if (channel != nChans)
						{
...
void LfpDisplayCanvas::paint(Graphics& g)
{
    ...
    {
        if (i == 5 || i == 0)
            g.drawLine(w/10*i+leftmargin,timescale->getHeight(),w/10*i+leftmargin,getHeight()-60-timescale->getHeight(),3.0f); //large tick
        else
            g.drawLine(w/10*i+leftmargin,timescale->getHeight(),w/10*i+leftmargin,getHeight()-60-timescale->getHeight(),1.0f); //small tick
    }
...
void LfpDisplay::refresh()
{
    ...
    if (canvas->fullredraw)
    {
        ...
        // Only update the dirty part for performance
...
//    for (int i = 0; i < drawableChannels.size(); ++i)
    {
        ...
        int componentTop = channels[i]->getY(); // Y axis goes from top to bottom
