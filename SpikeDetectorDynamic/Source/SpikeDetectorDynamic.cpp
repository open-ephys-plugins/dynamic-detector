...
bool SpikeDetectorDynamic::detectSpike(float sample, float threshold, String dSign)
{
    // check if a spike is detected
    if (dSign == "-ve") {
...
void SpikeDetectorDynamic::setDetectionSign(String dSign)
{
    for (int i = 0; i < detectionSign.size(); i++) {
        if (dSign == detectionSign[i]) {
            curDetectionSign = detectionSign[i];
            std::cout << "Setting detection  sign to " << dSign << std::endl;
            return;
        }
    }

    std::cout << "Error: " << dSign << " not supported" << std::endl;
}
                                              ...
                                              int64& timestamp)
{
    ...
    //String fn = String("spike_waveform_") + String(timestamp) +String("_") + String(peakIndex) +
    //       String("_") + String(electrodeNumber) + String("_") + String(currentChannel) + String(".txt");
    ...
    if (isChannelActive (electrodeNumber, currentChannel))
    {
        ...
        for (int sample = 0; sample < spikeLength; ++sample)
        {
            ...
            // getNextSample depends on the sampleIndex
            float data_sample = getNextSample(*(electrodes[electrodeNumber]->channels + currentChannel));

            //output.writeText(String(data_sample) + String("\n"),false,false);
...
int SpikeDetectorDynamic::getSpikePeakIndex(int index, int curChannel, SimpleElectrode* electrode)
{
    //find the peak index of of the threshold crossing point 

    int peakIndex = index;
    while (abs(getSampleFromBuffer(curChannel,index)) < abs(getSampleFromBuffer(curChannel, index+1)))
    {
        index++;		// Keep going until finding the largest point or peak
    }
    peakIndex = index;
    float peak_amp = abs(getSampleFromBuffer(curChannel, index));
    ...
    // note this will only search for the maximum point, it doesn't care whether it is a peak or not

    //int num_samples = electrode->prePeakSamples + electrode->postPeakSamples;
    //std::cout << "peakIndex before correction " << peakIndex << std::endl;
    int num_samples = electrode->postPeakSamples / 2; //if the second peak is far away, then probably it belongs to another neuron
    int current_test_sample = 1;
    while (current_test_sample < num_samples)
    {
        if (peak_amp > abs(getSampleFromBuffer(curChannel, index+1)))
        {
            current_test_sample++;
            index++;
        }
        else
        {
            ...
            peakIndex = index;
            peak_amp = abs(getCurrentSample(curChannel));
            index++;
            current_test_sample = 1;
        }
    }
    //std::cout << "peakindex after correction " << peakIndex << std::endl;
    return peakIndex;
}
            ...
            while (samplesAvailable(nSamples))
            {
                ...
                for (int chan = 0; chan < electrode->numChannels; chan++)
                {
                    if (*(electrode->isActive + chan))
                    {
                        ...
                        // check if a spike has been detected
                        float curSample = getNextSample(currentChannel);
                        isSpikeDetected = detectSpike(curSample, dyn_thresholds[chan][window_number], curDetectionSign);
                        //isSpikeDetected = abs(getNextSample(currentChannel)) > dyn_thresholds[chan][window_number];

                        //auto ts = getTimestamp(electrode->channels[0])+sampleIndex; //only for debugging purpose
...
void SpikeDetectorDynamic::computeSimpleThreshold(SimpleElectrode* electrode, std::vector<std::vector<float>>& thresholds)
{
...
void SpikeDetectorDynamic::AddSpikeEvent(int& peakIndex, SimpleElectrode* electrode, int& electrodeIdx)
{
    ...
    // force correction for debug
  /*  auto ts_correct = timestamp / 15000 * 15000;
    int peakIndex = peakIndex2 - (timestamp - ts_correct);*/

    sampleIndex = peakIndex - (electrode->prePeakSamples - 1); //at the beginning of spike
    //sample index will be used in addWaveformToSpikeObject later to access the raw data
